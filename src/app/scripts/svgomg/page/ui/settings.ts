...
export default class Settings extends EventEmitter {
  container: any;
  private _throttleTimeout: any;
  private _pluginInputs: any;
  private _globalInputs: any;
  private _sliderMap: any;
  ...
  constructor() {
    ...
    domReady.then(() => {
      this._pluginInputs = Array.from(document.querySelectorAll('.settings .plugins input'));
      this._globalInputs = Array.from(document.querySelectorAll('.settings .global input'));

      // map real range elements to Slider instances
      this._sliderMap = new WeakMap();

      // enhance ranges
      Array.from(document.querySelectorAll('.settings input[type=range]')).forEach(el =>
        this._sliderMap.set(el, new MaterialSlider(el)),
      );
  ...
  getSettings() {
    ...
    (output as any).fingerprint = fingerprint.join();
