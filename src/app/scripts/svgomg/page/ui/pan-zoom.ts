...
export default class PanZoom {
  private _target: any;
  private _shouldCaptureFunc: any;
  private _dx: number;
  private _dy: number;
  private _scale: number;
  private _active: number;
  private _lastPoints: any;
  ...
  constructor(target, { eventArea = target, shouldCaptureFunc = () => true } = {}) {
    ...
    // bind
    ['_onPointerDown', '_onPointerMove', '_onPointerUp'].forEach(funcName => {
  ...
  _update() {
    this._target.style.WebkitTransform = this._target.style.transform = `translate3d(${this
      ._dx}px, ${this._dy}px, 0) scale(${this._scale})`;
  }
